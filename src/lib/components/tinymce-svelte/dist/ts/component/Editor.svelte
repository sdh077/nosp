...
<script lang="ts">
  ...
  export let cssClass: string = 'tinymce-wrapper form-control';
  ...
  onMount(() => {
    if (getTinymce() !== null) {
      init({selector: 'textarea',content_css: 'dark'});
    } else {
...
{:else}
        <textarea id={id} bind:this={element} style="visibility:hidden" class="form-control bg-black"></textarea>
{/if}
</div>